(function() {
	...
	angular
		...
		.controller('${class.name}Controller', ${class.name}Controller);

	${class.name}Controller.$inject = ['$scope', '$location', '$routeParams', '$uibModal',
	...
	function ${class.name}Controller($scope, $location, $routeParams, $uibModal,
	<#list class.properties as property><#if property.upper == 1 && property.association == true>${property.name}Service, </#if></#list> 
	<#list class.properties as property><#if property.upper == -1 && property.association == true && property.next?? && property.next == true>${property.type?uncap_first}Service, </#if></#list>
	${class.name?uncap_first}Service) {
		...
		$scope.open = function () {
			...
			var modalInstance = $uibModal.open({
			      ...
			      controller: '${class.name}AddEditConfirmationController',
			      size : 'sm',
			      resolve: {
	...
	angular
		.module('${class.name?uncap_first}')
		.controller('${class.name}AddEditConfirmationController', ${class.name}AddEditConfirmationController);
	
	${class.name}AddEditConfirmationController.$inject = ['$scope', '$uibModalInstance', '${class.name?uncap_first}Service', '${class.name?uncap_first}'];
	function ${class.name}AddEditConfirmationController($scope, $uibModalInstance, ${class.name?uncap_first}Service, ${class.name?uncap_first}) {
